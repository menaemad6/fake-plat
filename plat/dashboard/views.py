...
from main.models import Profile , Lecture , StudentLectureObject, LectureCode ,LectureDiscount ,  Part , StudentPartObject , Chapter , ChapterLecture ,ChapterMember , Group , GroupMember , GroupLecture , GroupMessage ,GroupRequest , BuyLesson , BuyChapter
from main.models import Code , Notification , Transaction , LikeLecture , StudentQuestion ,  StudentQuestionAnswer , SocialLink
...
from main.models import PlatformSettings , View
...
from main.views import ready as ready_k

instructor_username = instructor_username_k
if User.objects.filter(username=instructor_username).first():
...
def other_settings(request):
    ...
    if request.method == 'POST':
        ...
        if type == 'leaderboard':
            ...
            if platform != '':
                platform .leaderboard = quest_bol
                platform.save()
                
            return redirect('/dashboard' )
        
        if type == 'reset-platform':

            if user_profile.instructor == True:
                delete_lectures = Lecture.objects.all().delete()
                delete_StudentLectureObject = StudentLectureObject.objects.all().delete()
                delete_LectureCode = LectureCode.objects.all().delete()
                delete_LectureDiscount = LectureDiscount.objects.all().delete()
                delete_Part = Part.objects.all().delete()
                delete_StudentPartObject = StudentPartObject.objects.all().delete()
                delete_SocialLink = SocialLink.objects.all().delete()
                delete_View = View.objects.all().delete()
                delete_Chapter = Chapter.objects.all().delete()
                delete_ChapterLecture = ChapterLecture.objects.all().delete()
                delete_ChapterMember = ChapterMember.objects.all().delete()
                delete_Group= Group.objects.all().delete()
                delete_GroupMember = GroupMember.objects.all().delete()
                delete_GroupLecture = GroupLecture.objects.all().delete()
                delete_GroupMessage = GroupMessage.objects.all().delete()
                delete_GroupRequest = GroupRequest.objects.all().delete()
                delete_BuyLesson = BuyLesson.objects.all().delete()
                delete_BuyChapter = BuyChapter.objects.all().delete()
                delete_Code = Code.objects.all().delete()
                delete_Notification = Notification.objects.all().delete()
                delete_Transaction = Transaction.objects.all().delete()
                delete_LikeLecture = LikeLecture.objects.all().delete()
                delete_StudentQuestion = StudentQuestion.objects.all().delete()
                delete_StudentQuestionAnswer = StudentQuestionAnswer.objects.all().delete()
                delete_Assignment = Assignment.objects.all().delete()
                delete_AssignmentOpen = AssignmentOpen.objects.all().delete()
                delete_AssignmentSubmit = AssignmentSubmit.objects.all().delete()
                delete_Question = Question.objects.all().delete()
                delete_Answer = Answer.objects.all().delete()
                delete_LoginInfo = LoginInfo.objects.all().delete()
                delete_Theme = Theme.objects.all().delete()
...
def dashboard_questions(request ):
    ...
    return render(request, 'dark/questions.html', {'modal_status':modal_status , 'specific_question':specific_question , 'user_profile': user_profile, 'new_questions':new_questions_objects , 'old_questions':old_questions_objects ,  'notifications' : notifications_count })
...
def dashboard_chapters(request ):
    ...
    chapters = Chapter.objects.filter(user=request.user).order_by('-last_update')

    if request.method == 'POST':
        chapter_name = request.POST['chapter-name']
        chapter_year = request.POST['chapter-year']
        chapter_price = request.POST['chapter-price']
        ...
        create_chapter = Chapter.objects.create(user=request.user , user_name=request.user.username , title=chapter_name , code=code , link=link , year=chapter_year , price=chapter_price )
        create_chapter.save()
        ...
        create_chapter.qr_code= 'qr-codes/' + str(create_chapter.link) + '.png'
        create_chapter.save()
...
def dashboard_chapter_details(request , slug):
    ...
    chapter_id = slug
    if Chapter.objects.filter(id=chapter_id).first():
        chapter = Chapter.objects.get(id=chapter_id)

        
        chapter_lectures = []
        chapter_lectures_objects = ChapterLecture.objects.filter(chapter_id=chapter_id).order_by('-added_at')
        for x in chapter_lectures_objects:
            if Lecture.objects.filter(id=x.lecture_id).first():
                lecture = Lecture.objects.get(id=x.lecture_id)
                chapter_lectures.append(lecture)

        chapter_students = ChapterMember.objects.filter(chapter_id=chapter_id).order_by('-created_at')

        all_students = Profile.objects.filter(instructor=False , admin=False)
        students_that_isnt_in_the_chapter = []
        for x in all_students:
            if ChapterMember.objects.filter(user=x.user , chapter_id=chapter.id).first():
                do_nothing = ''
            else:
                students_that_isnt_in_the_chapter.append(x)

        all_lectures = Lecture.objects.all().order_by('-created_at')
        lectures_that_isnt_in_the_chapter = []
        for x in all_lectures:
            if ChapterLecture.objects.filter(lecture_id=x.id , chapter_id=chapter.id).first():
                do_nothing = ''
            else:
                lectures_that_isnt_in_the_chapter.append(x)
    ...
    return render(request, 'dark/chapter.html', {'user_profile': user_profile, 'chapter':chapter , 'chapter_lectures':chapter_lectures , 'chapter_students':chapter_students ,'students_that_isnt_in_the_chapter':students_that_isnt_in_the_chapter, 'lectures_that_isnt_in_the_chapter':lectures_that_isnt_in_the_chapter ,  'notifications':notifications_count})
...
def chapter_functions(request):
    ...
    if request.method == 'POST':
        ...
        if type == 'settings':
            ...
            chapter_price = request.POST.get('chapter-price')
            chapter_year = request.POST.get('chapter-year')

            chapter = Chapter.objects.get(id=chapter_id)

            chapter.title = chapter_name
            chapter.year = chapter_year
            chapter.price = chapter_price
            chapter.save()

            chapter_members = ChapterMember.objects.filter(chapter_id=chapter_id)

            for x in chapter_members:
                x.chapter_title = chapter_name
                x.save()
                

            redirction_link = chapter.id
            return redirect('/dashboard/chapters/' + str(redirction_link))
        ...
        if type == 'delete':
            ...
            chapter = Chapter.objects.get(id=chapter_id)
            chapter_members = ChapterMember.objects.filter(chapter_id=chapter_id).delete()
            chapter_lectures = ChapterLecture.objects.filter(chapter_id=chapter_id).delete()
            chapter.delete()
        ...
        if type == 'add-lecture':
            ...
            if ChapterLecture.objects.filter(chapter_id=chapter_id , lecture_id=lecture_id).first():
                redirction_link = chapter.id
                return redirect('/dashboard/chapters/' + str(redirction_link))
            else:
                add_lecture = ChapterLecture.objects.create(chapter_id=chapter.id , lecture_id=lecture_id , user=request.user )
                add_lecture.save()
                add_lecture.added_at = datetime.now()
                add_lecture.save()
                ...
                chapter_members_objects = ChapterMember.objects.filter(chapter_id=chapter.id)


                for x in chapter_members_objects:
                    if BuyLesson.objects.filter(user=x.user , lecture_id=lecture.id).first():
                        nothing = 'Do Nothing'
                    else:
                        purchase_lesson = BuyLesson.objects.create(user=x.user , user_name=x.member_name ,user_image=x.image , lecture_id=lecture.id , method='chapter' , lecture_title=lecture.title)
        ...
        if type == 'remove-lecture':
            ...
            remove_lecture = ChapterLecture.objects.get(chapter_id=chapter.id , lecture_id=lecture_id)
        ...
        if type == 'purchase':
            ...
            chapter_lectures = []
            chapter_lectures_objects = ChapterLecture.objects.filter(chapter_id=chapter.id)
            for x in chapter_lectures_objects:
                if Lecture.objects.filter(id=x.lecture_id).first():
                    lecture = Lecture.objects.get(id=x.lecture_id)
                    chapter_lectures.append(lecture)
            ...
            buy_filter = ChapterMember.objects.filter(chapter_id=chapter.id, user=request.user).first()
            if buy_filter == None:
                if user_profile.money < chapter.price:
                    messages.warning(request, 'للاسف ليس لديك رصيد كافي في المحفظة لشراء هذا الشابتر')
                    return redirect('/profile/'+ str(request.user.username) + '/chapter/' + str(chapter_id))
                else:
                    serial = chapter.no_of_buys + 21000
                    purchase_chapter = BuyChapter.objects.create(user=request.user , chapter_id=chapter.id, chapter_title=chapter.title , chapter_value=chapter.price , user_name=user_profile.name , user_image=user_profile.image , method='wallet' , serial=serial)
                    purchase_chapter.save()

                    create_chapter_member = ChapterMember.objects.create(user=request.user , user_name=request.user.username , member_name=user_profile.name , image=user_profile.image , chapter_id=chapter.id , chapter_title=chapter.title , method='wallet' )
                    create_chapter_member.save()


                    user_profile.money = user_profile.money - chapter.price
                    ...
                    chapter_instructor_profile = Profile.objects.get(user=chapter.user)
                    chapter_instructor_profile.no_of_sells = chapter_instructor_profile.no_of_sells + 1
                    chapter_instructor_profile.save()
                    

                    invoice = Transaction.objects.create(user=request.user , user_name=request.user.username , item_id = chapter.id , item_title=chapter.title ,  value=chapter.price , wallet=user_profile.money , transaction_type='purchase' , purchase_type='wallet' , item_type='chapter' )
                    invoice.save()


                    for x in chapter_lectures:
                        if BuyLesson.objects.filter(user=request.user , lecture_id=x.id).first():
                            nothing = 'Do Nothing'
                        else:
                            purchase_lecture_for_member = BuyLesson.objects.create(user=request.user , user_name=user_profile.name ,user_image=user_profile.image , lecture_id=x.id , lecture_title=x.title , method='chapter' )
                            purchase_lecture_for_member.save()

                            lecture = Lecture.objects.get(id=x.id)
                            lecture.no_of_buys = lecture.no_of_buys + 1
                            lecture.save()

                            user_lecture_object = StudentLectureObject.objects.get(lecture_id=x.id , user=request.user)
                ...
                messages.success(request, 'انت مشتري هذا الشابتر بالفعل')
                return redirect('/profile/'+ str(request.user.username) + '/chapter/' + str(chapter_id))
        ...
        if type == 'add-member':
            ...
            if ChapterMember.objects.filter(user=member_object , chapter_id=chapter_id).first():
                redirction_link = chapter.id
                return redirect('/dashboard/chapters/' + str(redirction_link))
            else:

                serial = chapter.no_of_buys + 21000
                purchase_chapter = BuyChapter.objects.create(user=member_object , chapter_id=chapter.id, chapter_title=chapter.title , chapter_value=chapter.price , user_name=member_profile.name , user_image=member_profile.image , method='admin' , serial=serial)
                purchase_chapter.save()
            
                new_member = ChapterMember.objects.create(user=member_object, user_name=member_object.username , member_name=member_profile.name , chapter_id=chapter_id , chapter_title=chapter.title , image=member_profile.image ,method='admin')
                new_member.save()
                ...
                for x in chapter_lectures:
                    if BuyLesson.objects.filter(user=member_object , lecture_id=x.id).first():
                        nothing = 'Do Nothing'
                    else:
                        purchase_lecture_for_member = BuyLesson.objects.create(user=member_object , user_name=member_profile.name ,user_image=member_profile.image , lecture_id=x.id , lecture_title=x.title , method='chapter' )
                        purchase_lecture_for_member.save()
                        ...
                        user_lecture_object = StudentLectureObject.objects.get(lecture_id=x.id , user=member_object)
                        user_lecture_object.purchased = True
                        user_lecture_object.save()
        ...
        if type == 'remove-member':
            ...
            chapter_member_object = ChapterMember.objects.get(user=member_object, chapter_id=chapter.id)
            chapter_member_object.delete()

            chapter_purchase_object = BuyChapter.objects.get(user=member_object, chapter_id=chapter.id)
            chapter_purchase_object.delete()
...
def chapter_code_join_function(request):
    if User.objects.filter(username=request.user.username).first():
        user_object = User.objects.get(username=request.user.username)
        user_profile = Profile.objects.get(user=user_object)
    ...
    if request.method == 'POST':
        ...
        if Chapter.objects.filter(id=chapter_id).first():
            ...
            return redirect('/profile/'+ str(request.user.username) + '/chapters' )
        ...
        if chapter.code == code:
            ...
            if buy_filter == None:
                serial = chapter.no_of_buys + 21000
                purchase_chapter = BuyChapter.objects.create(user=request.user , chapter_id=chapter.id, chapter_title=chapter.title , chapter_value=chapter.price , user_name=user_profile.name , user_image=user_profile.image , method='chapter_code' , serial=serial)
                purchase_chapter.save()

                create_chapter_member = ChapterMember.objects.create(user=request.user , user_name=request.user.username , member_name=user_profile.name , image=user_profile.image , chapter_id=chapter.id , chapter_title=chapter.title , method='code' )
                create_chapter_member.save()
                ...
                invoice = Transaction.objects.create(user=request.user , user_name=request.user.username , item_id = chapter.id , item_title=chapter.title ,  value=chapter.price , wallet=user_profile.money , transaction_type='purchase' , purchase_type='code' , item_type='chapter' )
                invoice.save()
                ...
                messages.warning(request, 'انت مشتري الشابتر بالفعل')
                return redirect('/profile/'+ str(request.user.username) + '/chapter/' + str(chapter_id))

        else:
            messages.warning(request, 'الكود الذي ادخلته خطأ')
            return redirect('/profile/'+ str(request.user.username) + '/chapter/' + str(chapter_id) )
...
@login_required(login_url='login')
def chapter_join_link(request , chapter_link):
    if User.objects.filter(username=request.user.username).first():
        user_object = User.objects.get(username=request.user.username)
        user_profile = Profile.objects.get(user=user_object)
    ...
    if Chapter.objects.filter(link=link).first():
        chapter = Chapter.objects.get(link=link)
        ...
        if Chapter.objects.filter(link=link).first():
            if ChapterMember.objects.filter(user=profile_object , chapter_id=chapter.id).first():
                messages.info(request, 'انت بالفعل مشتري هذا الشابتر')
                return redirect('/profile/' + str(profile_object.username) + '/chapter/' + str(chapter.id))
            else:

                page_type = request.GET.get('page')
                if str(page_type) == 'view':
                    return render(request, 'profile/chapter-join.html' , {'chapter':chapter ,'invitation_type':type , 'user_profile': user_profile ,'profile': profile, 'profile_object': profile_object, 'notifications' : notifications_count })
                else:
                    if str(page_type) == 'join':

                        serial = chapter.no_of_buys + 21000
                        purchase_chapter = BuyChapter.objects.create(user=request.user , chapter_id=chapter.id, chapter_title=chapter.title , chapter_value=chapter.price , user_name=user_profile.name , user_image=user_profile.image , method=type , serial=serial)
                        purchase_chapter.save()
                    
                        new_member = ChapterMember.objects.create(user=request.user, user_name=request.user.username , member_name=user_profile.name , chapter_id=chapter.id , chapter_title=chapter.title , image=user_profile.image ,method=type)
                        new_member.save()
                        ...
                        messages.success(request, 'تم شراء الشابتر بنجاح')
                        return redirect('/profile/' + str(profile_object.username) + '/chapter/' + str(chapter.id))
            
                    else:
                        return redirect('/profile/' + str(profile_object.username) + '/chapters')
        else:
    ...
    if request.method == 'POST':
        ...
        if type == 'video':
            ...
            if len(request.FILES) != 0:
                ...
                video_type = request.POST.get('video-type')


                if str(video_type) == 'youtube':
                    youtube_link = request.POST.get('youtube-link')
                    new_part = Part.objects.create(youtube_url=youtube_link , lecture_id=lecture_id , user=request.user , user_name=request.user.username , type='video'  , title=title ,  part_number=part_number , duration=duration ,  views_limit=views , visible=True , original=True)
                    new_part.save()
                else:
                    if str(video_type) == 'player':
                        player_link = request.POST.get('player-link')
                        new_part = Part.objects.create(video_url=player_link , lecture_id=lecture_id , user=request.user , user_name=request.user.username , type='video'  , title=title ,  part_number=part_number , duration=duration ,  views_limit=views , visible=True , original=True)
                        new_part.save()
                    else:
                        if str(video_type) == 'customized-youtube':
                            custom_youtube_url = request.POST.get('custom_youtube_url')
                            new_part = Part.objects.create(custom_youtube_url=custom_youtube_url , lecture_id=lecture_id , user=request.user , user_name=request.user.username , type='video'  , title=title ,  part_number=part_number , duration=duration , views_limit=views , visible=True , original=True)
                            new_part.save()
                        else:
                            return redirect('/lecture/' + redirction_link)
