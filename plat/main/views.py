...
if ready == True:
    ...
    instructor_username = 'tarek-elfahd'
    platform = ''
...
def testing_page(request):
    ...
    url = 'lReL5FRFaIc'
    return render(request , 'testing-plyr.html' , {'url':url})
...
def index(request):
    ...
    if ready == True:
        if PlatformSettings.objects.filter(instructor_user=instructor_user).first():
            platform_settings_object = PlatformSettings.objects.get(instructor_user=instructor_user)
        else:
            platform_settings_object = ''  

        if platform_settings_object.leaderboard == True:

            students_profiles = Profile.objects.all()
            for x in students_profiles:
                student_true_answers_count = Answer.objects.filter(user=x.user , true=True).count()
                x.true_answers = student_true_answers_count
                x.save()

            students_profiles_ordered = students_profiles.order_by('-true_answers')
        else:
            students_profiles_ordered = ''
    else:
        platform_settings_object = ''
        students_profiles_ordered = ''
    ...
    return render(request, 'main/main.html', { 'lectures':lectures   , 'user_profile': user_profile ,  'notifications' : notifications_count , 'platform':platform_settings_object , 'students_profiles':students_profiles_ordered})
        ...
        if request.method == 'POST':
            ...
            if password == password2:
                if User.objects.filter(email=email).exists():
                    ...
                    for z in parts:
                        create_part_object = StudentPartObject.objects.create(part_id=z.part_id , lecture_id=z.lecture_id ,assignment_id=z.assignment_id ,assignment_questions_number=z.assignment_questions_number , user=request.user , user_name=request.user.username , type=z.type  , title=z.title , part_number=z.part_number , pdf_file_pages_number=z.pdf_file_pages_number ,duration=z.duration , views_limit=z.views_limit , visible=z.visible)
...
def grade(request , grade):
    ...
    if grade == '1':
        chapters = Chapter.objects.filter(year='first')
    else:
        if grade == '2':
            chapters = Chapter.objects.filter(year='second')
        else:
            if grade == '3':
                chapters = Chapter.objects.filter(year='third')
            else:
                return redirect("/lectures")
    ...
    if chapters.count() != 0:
        if user_profile:
            if user_profile.instructor == False:
                for x in chapters:
                    if BuyChapter.objects.filter(chapter_id=x.id , user=request.user):
                        x.purchased = True
                        x.save()
                    else:
                        x.purchased = False
    ...
    return render(request, 'lectures/lectures.html' , {'grade':grade_number , 'user_profile': user_profile , 'notifications' : notifications_count ,'lectures' : lectures , 'chapters':chapters , 'platform':platform})
...
def lecture(request , id):
    ...
    if all_students_number == 0:
        subscribed_students_percentage = 0
    else:
    ...
    if request.method == 'POST':
        ...
        if buy_filter == None:
            if user_profile.money < lecture.price:
               ...
               invoice = Transaction.objects.create(user=request.user , user_name=request.user.username , item_id = lecture.id , item_title=lecture.title ,  value=lecture.price , wallet=user_profile.money , transaction_type='purchase' , purchase_type='wallet' ,item_type='lecture' )
               invoice.save()
            ...
            messages.success(request, 'انت مشتري هذة المحاضرة بالفعل')
            return redirect('/lecture/'+ str(lecture.id) )
        ...
        if str(function_type) == 'confirm':
            ...
            if buy_filter == None:
                if user_profile.money < new_lecture_price:
                    ...
                    invoice = Transaction.objects.create(user=request.user , user_name=request.user.username , item_id = lecture.id , item_title=lecture.title ,  value=new_lecture_price , wallet=user_profile.money , transaction_type='purchase' , purchase_type='wallet_discount' ,item_type='lecture')
                    invoice.save()
