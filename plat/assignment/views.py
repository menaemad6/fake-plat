...
def lecture_assignment(request , id , assignment):
    ...
    student_answer_object_for_instructor = ''
    student_answers_for_review_for_instructor = ''

    # Correct Writing Questions 
    current_answer_object_for_instructor = '' 
    writing_answers_for_review_for_instructor = ''     
    ...
    if User.objects.filter(username=request.user.username).first():
        if user_profile.instructor == True:
            ...
            # View Answers 
            view_answers_request = request.GET.get('view-answers-for-instructor')
            if str(view_answers_request) == 'true':
                student_submit_serial = request.GET.get('serial')
                if AssignmentSubmit.objects.filter(serial=student_submit_serial).first():
                    student_submit_object = AssignmentSubmit.objects.get(serial=student_submit_serial)
                    student_answers_for_review_for_instructor = Answer.objects.filter(assignment_id=assignment.assignment_id , user=student_submit_object.user)

                    if len(str(request.GET.get('question'))) != 0:
                        question_id_for_review = request.GET.get('question')
                        if Answer.objects.filter(question_id=question_id_for_review , user=student_submit_object.user).first():
                            student_answer_object_for_instructor = Answer.objects.get(question_id=question_id_for_review , user=student_submit_object.user)
                        else:
                            student_answer_object_for_instructor = Answer.objects.get(assignment_id=assignment.assignment_id , question_number=1 , user=student_submit_object.user)
                    else:
                        student_answer_object_for_instructor = Answer.objects.get(assignment_id=assignment.assignment_id , question_number=1 , user=student_submit_object.user)
                    
                    student_answer_object_for_instructor.submittion_serial = student_submit_serial
                    student_answer_object_for_instructor.save()

                else:

                    return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part.part_id))
                



            # Correct Writing Answers 
            correct_answers_request = request.GET.get('correct-answers-for-instructor')
            writing_question_id = request.GET.get('writing-question')
            if str(correct_answers_request) == 'true':
                submitions_for_correction = applicants
                writing_answers_for_review_for_instructor = []
                ...
                for x in submitions_for_correction:
                    submit_object = x
                    serial = x.serial
                    student_answer_for_correction = Answer.objects.get(assignment_id=assignment.assignment_id , user=x.user , question_id=writing_question_id)

                    student_answer_for_correction.count = counter
                    student_answer_for_correction.save()
                    counter = counter + 1
                    writing_answers_for_review_for_instructor.append(student_answer_for_correction)

                if len(str(request.GET.get('current-answer'))) != 0:
                    current_answer_id_for_review = request.GET.get('current-answer')
                    if Answer.objects.filter(question_id=writing_question_id , answer_id=current_answer_id_for_review).first():
                        current_answer_object_for_instructor = Answer.objects.get(question_id=writing_question_id , answer_id=current_answer_id_for_review)
                        current_answer_object_for_instructor.submittion_serial = AssignmentSubmit.objects.get(user=current_answer_object_for_instructor.user , assignment_id=assignment.assignment_id).serial

                        current_answer_count = writing_answers_for_review_for_instructor.index(current_answer_object_for_instructor) + 1
                        if current_answer_count == len(writing_answers_for_review_for_instructor):
                            current_answer_object_for_instructor.next_answer = ''
                        else:
                            current_answer_object_for_instructor.next_answer = writing_answers_for_review_for_instructor[current_answer_count].answer_id

                        current_answer_object_for_instructor.save()
                    else:
                        return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part.part_id) + '?correct-answers-for-instructor=true&writing-question=' + str(writing_question_id) + '&current-answer=' + str(writing_answers_for_review_for_instructor[0].answer_id))
                else:
                    return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part.part_id) + '?correct-answers-for-instructor=true&writing-question=' + str(writing_question_id) + '&current-answer=' + str(writing_answers_for_review_for_instructor[0].answer_id))
    ...
    if request.method == 'POST':
        ...
        if str(function_type) == 'start-assignment':
            ...
            if AssignmentOpen.objects.filter(user=request.user , assignment_id=assignment_id).first():
                ...
                for x in assignment_questions:
                    create_answer = Answer.objects.create(assignment_id=assignment.assignment_id ,assignment_name=assignment.assignment_name , question_id=x.question_id ,question_number=x.question_number ,user=request.user ,user_name=user_profile.name ,question_type=x.question_type, question=x.question ,question_image=x.question_image ,question_true=x.true ,  answer1=x.answer1 , answer2=x.answer2 , answer3=x.answer3 , answer4=x.answer4 )
                    create_answer.save()
        ...
        if str(function_type) == 'delete-trial':
            ...
            if user_profile.instructor == True:
                ...
                assignment_open_object = AssignmentOpen.objects.get(user=assignment_submit_object.user , assignment_id=assignment_submit_object.assignment_id)
        ...
        if str(function_type) == 'navigate-answers':
            answer_id = request.POST['answer-id']
            submittion_serial = request.POST['submittion-serial']
            destination = request.POST['destination']


            if Answer.objects.filter(answer_id=answer_id).first():
                answer_object = Answer.objects.get(answer_id=answer_id)

                if answer_object.question_type == 'writing':
                    if len(request.POST['writing-question-correction']) != 0:
                        writing_question_correction_string = request.POST['writing-question-correction']
                        if str(writing_question_correction_string) == 'True':
                            writing_question_correction = True
                            writing_question_true = True
                        else:
                            writing_question_correction = True
                            writing_question_true = False
                    else: 
                        writing_question_correction = False
                        writing_question_true = False

                    answer_object.corrected = writing_question_correction
                    answer_object.true = writing_question_true
                    answer_object.save()

                    student_true_answers_count = Answer.objects.filter(true=True , assignment_id=assignment.assignment_id , user=answer_object.user).count()
                    student_false_answers_count = Answer.objects.filter(true=False , assignment_id=assignment.assignment_id , user=answer_object.user).count()
                    true_answers_percent = round(int(student_true_answers_count) / int(assignment.questions_count) * 100)
                    if AssignmentSubmit.objects.filter(serial=submittion_serial).first():
                        student_submittion_object = AssignmentSubmit.objects.get(serial=submittion_serial)
                        student_submittion_object.true_answers=student_true_answers_count
                        student_submittion_object.false_answers=student_false_answers_count
                        student_submittion_object.true_answers_percent=true_answers_percent
                        student_submittion_object.save()

                        # Update User Part 
                        student_assignment_part_object = StudentPartObject.objects.get(assignment_id=assignment.assignment_id , user=answer_object.user)
                        student_assignment_part_object.assignment_user_percentage = int(true_answers_percent)
                        student_assignment_part_object.save()
                        # Update User Part
                        ...
                        if assignment.award_system == True:
                            if int(true_answers_percent) >= int(assignment.award_percentage_needed):
                                if Lecture.objects.filter(id=assignment.award_lecture_id).first():
                                    award_lecture = Lecture.objects.get(id=assignment.award_lecture_id)
                                    student_submittion_object.award_lecture_name = award_lecture.title
                                    student_submittion_object.award_discount_value = assignment.award_discount_value
                                    student_submittion_object.award_lecture_id = assignment.award_lecture_id
                                    ...
                                    create_discount = LectureDiscount.objects.create(discount_id=pwd_for_discount , lecture_id=assignment.award_lecture_id , user=assignment.user , total_students_number=1 , discount_value=assignment.award_discount_value ,type='assignment_reward' , assignment_id=assignment.assignment_id , awarded_username=answer_object.user.username )
                                    create_discount.save()

                                    student_submittion_object.award_discount = create_discount.discount_id
                                    student_submittion_object.save()
                        # Award System 
                ...
                if len(request.POST['teacher-comment']) != 0:
                    teacher_comment = request.POST['teacher-comment']
                else: 
                    teacher_comment = ''

                answer_object.teacher_comment = teacher_comment
                answer_object.save()

                if str(destination) == 'same-question':
                    return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part_id) + '?view-answers-for-instructor=true&serial=' + str(submittion_serial) + '&question=' + str(answer_object.question_id) )
                else:
                    if Answer.objects.filter(question_id=destination , user=answer_object.user).first():
                        destination_question_id = Answer.objects.get(question_id=destination , user=answer_object.user).question_id
                        return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part_id) + '?view-answers-for-instructor=true&serial=' + str(submittion_serial) + '&question=' + str(destination_question_id) )
                    else:
                        return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part_id) + '?view-answers-for-instructor=true&serial=' + str(submittion_serial) + '&question=' + str(answer_object.question_id) )

            else:
                return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part_id) + '?view-answers-for-instructor=true&serial=' + str(submittion_serial) + '&question=1' )

        if str(function_type) == 'correct-writing-answers':
            answer_id = request.POST['answer-id']
            question_id = request.POST['question-id']
            submittion_serial = request.POST['submittion-serial']
            destination = request.POST['destination']
            ...
            if Answer.objects.filter(answer_id=answer_id).first():
                ...
                # Redirecting 
                if str(destination) == 'same-question':
                    return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part_id) + '?correct-answers-for-instructor=true&writing-question=' + str(answer_object.question_id)  + '&current-answer=' + str(answer_id))
                else:
                    if Answer.objects.filter(answer_id=destination).first():
                        return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part_id) + '?correct-answers-for-instructor=true&writing-question=' + str(answer_object.question_id)  + '&current-answer=' + str(destination))
                    else:
                        return redirect('/lecture/' + str(lecture.id) + '/assignment/' + str(part_id) + '?correct-answers-for-instructor=true&writing-question=' + str(question_id))
